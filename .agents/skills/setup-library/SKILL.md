---
name: setup-library
description: First-run setup for a freshly downloaded copy of the Library workspace template — personalise the agent instructions, create the user's real projects, and check everything works. Use when the user has just cloned or unzipped the Library template, says "set this up", "get me started", or "onboard me", or the README has no Active Projects section yet.
metadata:
...
1. **Check this is a Library workspace.** It needs `AGENTS.md`,
   `templates/`, and `.agents/skills/` at the root. If they're missing,
   this skill has been installed on its own: tell the user it sets up a
   copy of the template, which they can download from
   https://github.com/cody-eoai/library (Code → Download ZIP), and stop.
2. **Read** `AGENTS.md` and `README.md`. If `README.md` already has an
   `## Active Projects` section, setup has run before: ask what they want
   to change rather than starting over.
3. **How should agents work with you?** Ask about tone, how much pushback
...
4. **What are you working on?** For each piece of work, create it with the
...
5. **Remove the example?** Once there's at least one real project, offer to
   delete `projects/example-project/`. Only delete it if they say yes.
6. **Who do you work with?** Offer to create notes for key collaborators
   with `new-person`. Skip it if they'd rather not. Keep notes public-safe
   (see `people/README.md`).
7. **Knowledge base.** Explain in two sentences: drop material into
...
8. **Check it works.** Run `python -m pytest tests`. On Windows, a failing
   ...
   note.
9. **Version control.** If the folder isn't a git repo, offer to run
